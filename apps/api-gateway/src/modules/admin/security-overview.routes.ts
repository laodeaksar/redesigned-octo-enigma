...
import { desc, gte, count, sql, and } from "drizzle-orm";
 ...
 *   100+  → CRITICAL
 */
function threatLevel(failuresLastHour: number): "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" {
...
async function fetchAuditData(h1: Date, h24: Date, d7: Date) {
  ...
  const [
    ...
    // Counts per window
    db.select({ n: count() }).from(auditLogsTable).where(gte(auditLogsTable.createdAt, h1)),
    db.select({ n: count() }).from(auditLogsTable).where(gte(auditLogsTable.createdAt, h24)),
    db.select({ n: count() }).from(auditLogsTable).where(gte(auditLogsTable.createdAt, d7)),
...
async function fetchRedisData() {
  ...
  const nearThreshold: Array<{ ip: string; failures: number; threshold: number; pct: number }> = [];
  if (failKeys.length > 0) {
    ...
    for (let i = 0; i < failKeys.length; i++) {
      ...
      const n   = parseInt(counts[i] ?? "0", 10);
      const pct = Math.round((n / IP_BLOCK_THRESHOLD) * 100);
      if (pct >= 50) {
        nearThreshold.push({ ip, failures: n, threshold: IP_BLOCK_THRESHOLD, pct });
      }
    }
