...
const loadingIndicatorVariants = cva(
  "inline-flex animate-spin items-center justify-center rounded-full border-2 border-transparent",
  {
    variants: {
      size: {
...
export function ActionLoadingOverlay({
  ...
  return (
    <div className="relative">
      <div
        className={`transition-opacity duration-150 ${active ? "pointer-events-none opacity-50" : "opacity-100"}`}
      >
        {children}
      </div>
