...
app.get("/admin/blocked-ips", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  const redis = getRedis();
  if (!redis) return c.json(failure("SERVICE_UNAVAILABLE", "Redis not available"), 503);
  ...
  do {
    const [next, keys] = await redis.scan(cursor, "MATCH", `${BLOCK_KEY_PREFIX}*`, "COUNT", 200);
    cursor = next;
...
app.get("/admin/blocked-ips/:ip/history", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  const redis = getRedis();
  if (!redis) return c.json(failure("SERVICE_UNAVAILABLE", "Redis not available"), 503);
  ...
  if (blockRaw) {
    try { blockEntry = JSON.parse(blockRaw) as BlockEntry; } catch { /* ignore */ }
  }
  ...
  return c.json(success({
    ...
    blockEntry,
    failureCount:  failCount ? parseInt(failCount, 10) : 0,
...
// ── POST /admin/blocked-ips/:ip/block — manually block an IP ─────────────────
app.post("/admin/blocked-ips/:ip/block", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  const redis = getRedis();
  if (!redis) return c.json(failure("SERVICE_UNAVAILABLE", "Redis not available"), 503);
  ...
  try {
    const body = await c.req.json() as { durationSeconds?: number; reason?: string };
    if (body.durationSeconds) durationSeconds = Math.max(60, body.durationSeconds);
  ...
  console.warn(`[ip-blocklist] MANUAL BLOCK ${ip} by ${admin?.email ?? "admin"} until ${until.toISOString()}`);

  return c.json(success({ ip, ...entry }), 201);
});

// ── DELETE /admin/blocked-ips/:ip/unblock — remove a block ───────────────────
app.delete("/admin/blocked-ips/:ip/unblock", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  const redis = getRedis();
  if (!redis) return c.json(failure("SERVICE_UNAVAILABLE", "Redis not available"), 503);
  ...
  if (delBlock === 0) {
    return c.json(failure("NOT_FOUND", `IP ${ip} is not currently blocked`), 404);
  }

  console.info(`[ip-blocklist] UNBLOCKED ${ip} by ${admin?.email ?? "admin"}`);
