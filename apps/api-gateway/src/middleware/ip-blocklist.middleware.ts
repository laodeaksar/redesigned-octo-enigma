...
export const IP_BLOCK_THRESHOLD        = parseInt(process.env.IP_BLOCK_THRESHOLD        ?? "10",   10);
export const IP_BLOCK_WINDOW_SECONDS   = parseInt(process.env.IP_BLOCK_WINDOW_SECONDS   ?? "300",  10);
export const IP_BLOCK_DURATION_SECONDS = parseInt(process.env.IP_BLOCK_DURATION_SECONDS ?? "3600", 10);
...
export function extractIp(req: { header: (k: string) => string | undefined }): string {
  return (
...
export const ipBlocklistMiddleware = createMiddleware(async (c, next) => {
  ...
  if (!redis) return next(); // Redis unavailable — pass through

  const ip = extractIp(c.req);
  if (ip === "unknown") return next(); // Can't block unknown IPs

  const raw = await redis.get(`${BLOCK_KEY_PREFIX}${ip}`).catch(() => null);
  if (!raw) return next(); // Not blocked

  let entry: BlockEntry | null = null;
  try { entry = JSON.parse(raw) as BlockEntry; } catch { /* ignore */ }
...
export async function recordIpFailure(ip: string): Promise<number | null> {
  ...
  try {
    ...
    if (count >= IP_BLOCK_THRESHOLD) {
      ...
      await redis.set(blockKey, JSON.stringify(entry), "EX", IP_BLOCK_DURATION_SECONDS);
      await redis.del(failKey); // Reset counter

      console.warn(`[ip-blocklist] AUTO-BLOCKED ${ip} until ${until.toISOString()} (${count} failures)`);
    }
