  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 pt-2">
        <button
          className="flex-1 rounded-lg bg-gray-900 py-2.5 font-semibold text-sm text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
          disabled={loading}
          type="submit"
        >
        ...
        <button
          className="rounded-lg border border-gray-200 px-5 py-2.5 font-medium text-gray-700 text-sm transition-colors hover:bg-gray-50"
          disabled={loading}
          onClick={onCancel}
          type="button"
        >
...
function AddressCard({
  ...
  return (
    ...
    >
      {address.isDefault && (
        <span className="absolute top-3 right-3 rounded-full bg-gray-900 px-2 py-0.5 font-semibold text-[10px] text-white uppercase tracking-wide">
          Utama
        </span>
      )}
      ...
      <div className="mt-3 flex gap-2">
        <button
          className="rounded-lg border border-gray-200 px-3 py-1.5 font-medium text-gray-700 text-xs transition-colors hover:bg-gray-50"
          onClick={onEdit}
        >
          Edit
        </button>
        <button
          className="rounded-lg border border-red-100 px-3 py-1.5 font-medium text-red-600 text-xs transition-colors hover:bg-red-50 disabled:opacity-50"
          disabled={deleting}
          onClick={onDelete}
        >
...
export default function AddressManager({ initialAddresses, token }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 animate-fade-in rounded-full bg-gray-900 px-5 py-2.5 font-medium text-sm text-white shadow-lg">
          ✓ {toast}
        </div>
      )}
      ...
      {mode === "list" && (
        <>
          {addresses.length === 0 ? (
            <div className="rounded-xl border-2 border-gray-200 border-dashed p-10 text-center">
          ...
          <button
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-gray-200 border-dashed py-3 font-medium text-gray-500 text-sm transition-colors hover:border-gray-300 hover:text-gray-700"
            onClick={startAdd}
          >
            <svg
              className="h-4 w-4"
      ...
      {mode === "list" && addresses.length > 0 && (
        <div className="pt-2 text-center">
          <a
            className="font-medium text-gray-600 text-sm transition-colors hover:text-gray-900 hover:underline"
            href="/checkout"
          >
            Lanjut ke Checkout →
          </a>
