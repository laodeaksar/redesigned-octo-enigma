...
|-----------|-----|------|--------|
| **P0** | `apps/web` | Tambah `INTERNAL_API_URL` env — pisahkan dari `PUBLIC_API_URL` untuk SSR fetch di frontmatter | Server-to-server fetch lewat public domain menambah latency dan TTFB. Gunakan `http://localhost:3000` |
...
| **P0** | `apps/web` | Hapus `web-push`, `postgres`, `drizzle-orm` dari Vite SSR externals | Dependency ini tidak boleh ada di web frontend — cek dari mana kebocorannya |
...
| **P1** | `apps/web` | Ganti `window.removeFromCartWithUndo` dengan nanostores event/atom | Anti-pattern, rawan race condition |
| **P1** | `apps/web` | Jangan pass raw `token` ke island props — buat Astro API route `/api/proxy/[...path]` sebagai authenticated fetch relay | JWT tidak boleh ada di client-side props/bundle |
| **P1** | `apps/web` | Audit `client:load` vs `client:visible` vs `client:idle` per island | `RecentlyViewed`, `TrackProductView` tidak perlu `client:load` — bisa `client:idle` atau `client:visible` |
| **P1** | `packages/common` | Buat `src/types/storefront.ts` dengan tipe yang dishare antar apps | Type-safe end-to-end tanpa redefinisi per-app |
| **P2** | `api-gateway` | Tambah BFF aggregation endpoints `/bff/home` dan `/bff/pdp/:slug` | Kurangi jumlah SSR round-trip dari 2–3 serial/parallel fetch menjadi 1 |
| **P2** | `api-gateway` | Cache `GET /products` dan `GET /categories` di Redis TTL 30–60s | Endpoint paling sering dipanggil, aman di-cache karena data jarang berubah |
| **P2** | `apps/web` | Tambah `<link rel="prefetch">` untuk `/checkout` saat cart tidak kosong | Kurangi navigation latency ke halaman high-intent |
