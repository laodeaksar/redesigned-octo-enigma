...
import { getRedis, db, env } from "@/config";
...
export function meetsThreshold(level: ThreatLevel, threshold: ThreatLevel): boolean {
  return LEVEL_ORDER[level] >= LEVEL_ORDER[threshold];
}

// ── Colour / emoji helpers ────────────────────────────────────────────────────

const LEVEL_META: Record<ThreatLevel, { emoji: string; color: number; label: string }> = {
  LOW:      { emoji: "🟢", color: 0x2ecc71, label: "Low"      },
  MEDIUM:   { emoji: "🟡", color: 0xf39c12, label: "Medium"   },
  HIGH:     { emoji: "🔴", color: 0xe74c3c, label: "High"     },
  CRITICAL: { emoji: "🚨", color: 0x7b0000, label: "Critical" },
};
...
function buildWebhookPayload(ctx: AlertContext) {
  ...
  return {
    ...
    embeds: [{
      ...
      fields: [
        { name: "Failures / hour", value: String(ctx.failuresLastHour), inline: true },
        { name: "Blocked IPs",     value: String(ctx.activeBlocks),     inline: true },
    ...
    blocks: [
      ...
      },
      {
        ...
        fields: [
          ...
          { type: "mrkdwn", text: `*Failures (last hour):*\n${ctx.failuresLastHour}` },
          { type: "mrkdwn", text: `*Active IP Blocks:*\n${ctx.activeBlocks}` },
        ],
      },
      {
        type: "context",
        elements: [{ type: "mrkdwn", text: `Gateway: ${ctx.gatewayUrl} · ${ts}` }],
      },
    ],
  };
}
...
function buildEmailHtml(ctx: AlertContext): string {
  ...
  const bgColor = ctx.level === "CRITICAL" ? "#7b0000" :
                  ctx.level === "HIGH"     ? "#e74c3c" :
                  ctx.level === "MEDIUM"   ? "#f39c12" : "#2ecc71";
...
async function sendWebhook(ctx: AlertContext): Promise<"sent" | "failed" | "not_configured"> {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[alerting] Webhook responded ${res.status}: ${await res.text().catch(() => "")}`);
      return "failed";
    }
...
async function sendEmail(ctx: AlertContext): Promise<"sent" | "failed" | "not_configured"> {
  const to = env.ALERT_EMAIL_TO;
  if (!to) return "not_configured";

  const redis = getRedis();
  if (!redis) return "not_configured"; // BullMQ requires Redis

  try {
    const { Queue } = await import("bullmq");
    const { QUEUES }  = await import("@repo/common/events");

    type ConnectionOptions = ConstructorParameters<typeof Queue>[1]["connection"];
...
export async function sendAlert(ctx: AlertContext): Promise<AlertResult> {
  ...
  const cooldownActive = redis ? (await redis.exists(cooldownKey).catch(() => 0)) === 1 : false;
  ...
  if (anySent && redis) {
    ...
    await redis.set(LAST_SENT_KEY, JSON.stringify({
      level:     ctx.level,
      sentAt:    new Date().toISOString(),
      channels:  { webhook, email },
      context:   ctx,
    }), "EX", 86_400).catch(() => {}); // keep for 24h
  }
...
export async function updateTrackedLevel(
  ...
  const redis = getRedis();
  if (!redis) return { previous: "LOW", changed: false };

  const stored = (await redis.get(LAST_LEVEL_KEY).catch(() => null)) as ThreatLevel | null;
 ...
 */
export async function getLastSentAlert(): Promise<Record<string, unknown> | null> {
  const redis = getRedis();
