...
export const oauthRoutes = new Elysia({ prefix: "/auth/oauth" })
  ...
  .all("/callback/:provider", async ({ request }) => auth.handler(request), {
    params: t.Object({
      provider: t.Union([t.Literal("google"), t.Literal("github")]),
    }),
    detail: {
      tags: ["Auth"],
      summary: "OAuth callback (handled by Better-auth)",
    },
  })

  .all("/:provider", async ({ request }) => auth.handler(request), {
    params: t.Object({
      provider: t.Union([t.Literal("google"), t.Literal("github")]),
    }),
...
export const betterAuthHandler = new Elysia({ prefix: "/api/auth" }).all(
  "/*",
