...
export async function handleSecurityAlertEmail(
  ...
  const jobLogger = logger.child({ jobId, queueName: QUEUE_NAME, attempt: job.attemptsMade + 1 });
  ...
  try {
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("Email provider timeout after 15000ms")), 15_000)
      ),
    ]);
    ...
    emailSendDuration.observe({ type: QUEUE_NAME, provider: result.provider }, durationSec);

    const toStr = Array.isArray(data.to) ? data.to.join(", ") : data.to;
    jobLogger.info({ to: toStr, provider: result.provider, durationSec }, "Security alert email sent");
