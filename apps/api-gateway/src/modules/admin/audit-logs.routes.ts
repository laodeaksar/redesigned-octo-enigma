...
app.get("/admin/audit-logs", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  ...
  const conditions = [
    ...
    statusCode ? eq(auditLogsTable.statusCode,     parseInt(statusCode, 10)) : undefined,
...
app.get("/admin/audit-logs/stats", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  if (!db) return c.json(failure("SERVICE_UNAVAILABLE", "Database not available"), 503);
  ...
  const [byCode, byHour, topIps] = await Promise.all([
    ...
    db
      ...
      .from(auditLogsTable)
      .where(gte(auditLogsTable.createdAt, new Date(Date.now() - 86400_000)))
...
app.delete("/admin/audit-logs", requireAuth, requireRole("admin", "super_admin"), async (c) => {
  if (!db) return c.json(failure("SERVICE_UNAVAILABLE", "Database not available"), 503);

  const olderThanDays = Math.max(1, parseInt(c.req.query("olderThanDays") ?? "30", 10));
